...
- `LocalLlmService` は llama.cpp の model/context/sampler の lifetime を持ち、model load と generation を worker thread で実行します。
...
- Prompt input: `InputTextMultiline` と `Send` / `Clear` ボタン。
...
- `Stop()` は worker を join し、model/context/sampler を解放し、`Stopped` にします。
...
- structured JSON grammar を必要に応じて含む `llama_sampler` chain。

`Generate()` は context memory を clear し、model chat template を適用し、prompt を tokenize し、prompt token を decode したあと、`maxTokens` まで sampling します。response event には次の情報が入ります。
...
- Worker thread: llama.cpp model/context/sampler と event production。
