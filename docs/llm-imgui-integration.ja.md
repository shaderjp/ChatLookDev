...
`Generate()` は新しい `llama_sampler` chain を作成し、context memory を clear し、model chat template を適用し、prompt を tokenize し、prompt token を decode したあと、`maxTokens` まで sampling します。response event には次の情報が入ります。
